...
class PPO:
    ...
    def select_action(self, obs):
        obs = self.obs_normaliser(obs) # Normalise and update stats
        ...
        return action.item(), log_prob.item(), value.item(), obs # Returning norm_obs as 4th value
    ...
    def update(self):
        ...
        # Re-norm removed from here as obs stored are normalised!
...
def train_ppo(env, total_timesteps = 200000, rollout_steps=2048, log_dir='../logs'):
    ...
    while timestep < total_timesteps:
        for i in range(rollout_steps):
            action, log_prob, value, norm_obs = agent.select_action(obs)
            next_obs, reward, terminated, truncated, _ = env.step(action)
            done = terminated or truncated
            
            agent.store(norm_obs, action, reward, log_prob, value, done)
