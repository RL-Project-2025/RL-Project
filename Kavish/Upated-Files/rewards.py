...
def check_pumps_flow(readings: dict, pumps_list: list):
    """
    Compute flow penalty by summing flow of all pumps.
    Now accepts 'readings' and 'pumps_list' directly.
    """
    total_flow = 0
    lowest_flow = 0
    ...
    for pump in pumps_list:
        # Check if pump data exists in the readings
        if pump in readings and 'flow' in readings[pump]:
            total_flow += readings[pump]['flow']

    # Return max-min normalized flow
    return (total_flow - lowest_flow) / (highest_flow - lowest_flow)


def check_pumps_updates(step_updates: dict, pumps_list: list, simple=True):
    """
    ...
    if simple:
        for pump in pumps_list:
            # If the pump status changed (shown by 1), add penalty
            if step_updates.get(pump, 0) > 0:
